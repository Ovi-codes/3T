...
- **Endpoints:** `POST /api/auth/signup` (201, logs in), `POST /api/auth/login` (200), `GET /api/auth/me`
  (200 or 401). Authorisation is deny-by-default; the public API (events, anonymous registration,
  signup/login, ping, health) is enumerated in `SecurityConfig`.
...
- **CSRF:** Spring's CSRF token machinery is **off** for the JSON API — it's served same-origin and the
  ...
  hardening item, tracked alongside the Entra swap.**
