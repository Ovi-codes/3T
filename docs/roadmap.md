...
- Implement user roles - admins should be able to CRUD events — *in progress:* Increment 10a (#57)
  landed the role model (`ROLE_ADMIN` via `ADMIN_EMAILS`) + **create** an event; Increment 10b (#58)
  added **edit**, **delete** (only while nobody is registered), **cancel** (terminal, emails every
  registrant — see [ADR-0001](./adr/0001-cancel-vs-hard-delete-events.md)) and the admin-only
  registration count.
